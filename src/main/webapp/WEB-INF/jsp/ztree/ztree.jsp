  ...
  <head>
	...
	var dragId;
		...
		function beforeRename(treeId, treeNode, newName, isCancel){
			...
			var data = {id: treeNode.id,pId: treeNode.pId,name: newName};
			$.ajax({
				async: false,
				url:'<%=request.getContextPath()%>/updateTree.do',
				method:'post',
				dataType:'json',
				data:data,
		...
		function beforeRemove(treeId, treeNode){
			...
			$.ajax({
				async: false,
				url:'<%=request.getContextPath()%>/deleteTree.do',
				method:'post',
				dataType:'json',
				data:data,
				success:function(data){
		...
		function beforeDrop(treeId, treeNodes, targetNode, moveType){
			if(targetNode.pId == dragId){
				...
				$.ajax({
					async: false,
					url:'<%=request.getContextPath()%>/updateTreeSort.do',
					method:'post',
					dataType:'json',
					data:data, 
					success:function(data){
		...
		function addHoverDom(treeId, treeNode) {
			...
			if (btn) btn.bind("click", function(){
				var zTree = $.fn.zTree.getZTreeObj("treeDemo");
				zTree.addNodes(treeNode, {id:id, pId:treeNode.id, name:"新增XXX"});
				//var l = treeNode.children.length;
				//zTree.editName(treeNode.children[l]);
				var confirmVal = false;
				var data = {name: "新增XXX",pId: treeNode.id,level: treeNode.level + 2};
				$.ajax({
					async: false,
					url:'<%=request.getContextPath()%>/saveTree.do',
					method:'post',
					dataType:'json',
					data:data,
					success:function(data){
